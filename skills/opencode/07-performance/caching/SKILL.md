---
name: caching
description: Utiliser cette skill pour : Concevoir une stratégie de cache (navigateur, CDN, serveur, base de données) adaptée au type de données. S'applique aux projets web/mobile (catégorie « Performance » du pack Web Master AI, stack type Next.js, React Native, Supabase, Django, Electron). Déclencher dès qu'une tâche touche à « caching ».
---
...
## Objectif
Concevoir une stratégie de cache (navigateur, cdn, serveur, base de données) adaptée au type de données.

## Quand déclencher cette skill
- L'utilisateur demande explicitement de travailler sur « caching »
- La tâche en cours implique concevoir une stratégie de cache (navigateur, cdn, serveur, base de données) adaptée au type de données
...
## Livrables attendus
- Une solution concrète et fonctionnelle pour « caching », adaptée au projet en cours
