---
name: optimisation-requetes-sql
description: Utiliser cette skill pour : Diagnostiquer et corriger les requêtes lentes (plans d'exécution, N+1, index manquants). S'applique aux projets web/mobile (catégorie « Bases de données » du pack Web Master AI, stack type Next.js, React Native, Supabase, Django, Electron). Déclencher dès qu'une tâche touche à « optimisation des requêtes sql ».
---
...
## Objectif
Diagnostiquer et corriger les requêtes lentes (plans d'exécution, n+1, index manquants).

## Quand déclencher cette skill
- L'utilisateur demande explicitement de travailler sur « optimisation des requêtes sql »
- La tâche en cours implique diagnostiquer et corriger les requêtes lentes (plans d'exécution, n+1, index manquants)
...
## Livrables attendus
- Une solution concrète et fonctionnelle pour « optimisation des requêtes sql », adaptée au projet en cours
