---
name: render
description: Utiliser cette skill pour : Déployer une application sur Render (web service, base de données, cron jobs) avec configuration adaptée. S'applique aux projets web/mobile (catégorie « Déploiement » du pack Web Master AI, stack type Next.js, React Native, Supabase, Django, Electron). Déclencher dès qu'une tâche touche à « render ».
---
...
## Objectif
Déployer une application sur render (web service, base de données, cron jobs) avec configuration adaptée.

## Quand déclencher cette skill
- L'utilisateur demande explicitement de travailler sur « render »
- La tâche en cours implique déployer une application sur render (web service, base de données, cron jobs) avec configuration adaptée
...
## Livrables attendus
- Une solution concrète et fonctionnelle pour « render », adaptée au projet en cours
